...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path.includes('/api/divers/') && method === 'GET') {
      ...
      return data ? sendJson(res, 200, data) : sendError(res, 404, 'Not found');
    }
    ...
    if (path.includes('/api/bookings/stats/last30days') && method === 'GET') {
      ...
      return sendJson(res, 200, { booking_count, total_revenue, total_amount });
    }
    ...
    if (path.includes('/api/bookings') && method === 'POST' && !path.includes('/api/bookings/')) {
      ...
      if (!diver_id) return sendError(res, 400, 'diver_id is required');
      const { data, error } = await supabase
        .from('bookings')
        .insert([
          {
